...
GOOGLE_CODES_TO_LANGUAGES = {code: lang for lang,
                             code in GOOGLE_LANGUAGES_TO_CODES.items()}
    ...
    if out == "screen":
        ...
        for i, (lang_name, lang_code) in enumerate(languages.items(), 1):
            translated_text = TransLate(
                text, "auto", lang_code) if text else ""
            print(f"{i:<3} {lang_name.capitalize():<20} {
                  lang_code:<10} {translated_text:<30}")
        return "Ok"

    elif out == "file":
        try:
            with open("language_list.txt", "w") as file:
                file.write(f"{'N':<3} {'Language':<20} {
                           'ISO-639 code':<10} {'Text':<30}\n")
                file.write("-" * 65 + "\n")
                for i, (lang_name, lang_code) in enumerate(languages.items(), 1):
                    translated_text = TransLate(
                        text, "auto", lang_code) if text else ""
                    file.write(f"{i:<3} {lang_name.capitalize():<20} {
                               lang_code:<10} {translated_text:<30}\n")
            return "Ok"
        except Exception as e:
            return f"Error: {str(e)}"
